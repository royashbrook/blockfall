...
mod worldgen_tests {
    ...
    // Progressive detail streaming (#159): the base pass plus the deferred detail
    // pass must reproduce the combined generate() byte for byte, across many chunks
    // and seeds, and the detail pass must actually add content (base-only differs
    // from full for at least some chunk). This is the same-final-world guarantee
    // the streaming split relies on: only the ORDER of appearance changes, never
    // the resulting voxels, and it stays deterministic from the seed.
    #[test]
    fn detail_split_reproduces_full_world() {
        let dump = |g: &TerrainGen, c: ChunkCoord| -> Vec<BlockId> {
            let mut ch = DenseChunk::new(AIR);
            g.generate(c, &mut ch);
            let mut v = Vec::with_capacity((K_CHUNK_DIM * K_CHUNK_DIM * K_CHUNK_DIM) as usize);
            for lz in 0..K_CHUNK_DIM {
                for ly in 0..K_CHUNK_DIM {
                    for lx in 0..K_CHUNK_DIM {
                        v.push(ch.get(lx, ly, lz));
                    }
                }
            }
            v
        };
        let dump_split = |g: &TerrainGen, c: ChunkCoord| -> Vec<BlockId> {
            let mut ch = DenseChunk::new(AIR);
            g.generate_base(c, &mut ch);
            g.generate_detail(c, &mut ch);
            let mut v = Vec::with_capacity((K_CHUNK_DIM * K_CHUNK_DIM * K_CHUNK_DIM) as usize);
            for lz in 0..K_CHUNK_DIM {
                for ly in 0..K_CHUNK_DIM {
                    for lx in 0..K_CHUNK_DIM {
        ...
        let dump_base = |g: &TerrainGen, c: ChunkCoord| -> Vec<BlockId> {
            let mut ch = DenseChunk::new(AIR);
            g.generate_base(c, &mut ch);
            let mut v = Vec::with_capacity((K_CHUNK_DIM * K_CHUNK_DIM * K_CHUNK_DIM) as usize);
            for lz in 0..K_CHUNK_DIM {
        ...
        let mut saw_deferred_content = false;
        for seed in [11u64, 1, 42, 7, 1234] {
            let mut g = TerrainGen::new();
            g.seed(seed);
            for cx in -3..=3 {
                for cz in -3..=3 {
                    for cy in 0..=3 {
                        let c = ChunkCoord { x: cx, y: cy, z: cz };
                        let full = dump(&g, c);
                        let split = dump_split(&g, c);
                        assert_eq!(full, split, "seed {seed} chunk {c:?}: base+detail != generate");
                        let base = dump_base(&g, c);
                        if base != full {
                            saw_deferred_content = true;
                        }
                    }
                }
            }
        ...
        assert!(
            saw_deferred_content,
            "detail pass added nothing anywhere; deferral would be a no-op"
        );
    }
}
