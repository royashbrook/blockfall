...
mod tests {
    ...
    #[test]
    fn wood_beam_is_sky_light_pass_through() {
        let mut store = ChunkStore::new();
        let cc = ChunkCoord { x: 0, y: 0, z: 0 };
        let mut ch = PaletteChunk::new(cc, 0);
        ch.set(8, 8, 8, 51);
        ...
        assert_eq!(
            ch.sky_light(8, 8, 8),
            15,
            "shaped beam reads light from its cell"
        );
        assert_eq!(
            ch.sky_light(8, 7, 8),
            15,
            "inset timber must not cast a full-voxel shadow"
        );
    }

    #[test]
    fn chopping_block_is_sky_light_pass_through() {
        let mut store = ChunkStore::new();
        let cc = ChunkCoord { x: 0, y: 0, z: 0 };
        let mut ch = PaletteChunk::new(cc, 0);
        ch.set(8, 8, 8, 56);
        ...
        assert_eq!(
            ch.sky_light(8, 8, 8),
            15,
            "custom workstation mesh reads light from its cell"
        );
        assert_eq!(
            ch.sky_light(8, 7, 8),
            15,
